...
vec3 Material::getDiffuse(const vec2& texCoord) const {
    if (useTexture && texture && texture->valid()) return texture->value(texCoord);
    return diffuse;
}

vec3 Material::getSpecular(const vec2& texCoord) const {
    if (useSpecularTexture && specularTexture && specularTexture->valid())
        return specularTexture->value(texCoord);
    return specular;
}
